# main.py - Adapted for YOLO Integration
import numpy as np
import cv2
# import mediapipe as mp # REMOVED
# --- NEW IMPORTS for YOLO ---
...
from utils import GOOD_COLOR, BAD_COLOR, TEXT_COLOR, draw_yolo_skeleton, \
    YOLO_KEYPOINT_MAP  # ADDED YOLO_KEYPOINT_MAP and draw_yolo_skeleton

# --- Initialize YOLO Pose Model (REPLACING MediaPipe Pose) ---
try:
    # Use a pre-trained YOLOv8 pose model. Ensure this file exists or is downloaded.
...
# mp_drawing and pose initializations removed
...
class WorkoutAnalyzer:
    """Tracks workout metrics for analysis (Remains the same)"""
...
def display_analysis_summary(summary):
    """Display analysis summary in terminal (Remains the same)"""
    print("\n" + "=" * 60)
...
def run_live_mode(exercise_name):
    ...
    while cap.isOpened():
        ...
        image = frame.copy()  # Use a copy for drawing
        ...
        # Check for results and extract the *best* (highest confidence) person's pose
        if len(yolo_results[0].keypoints.data) > 0:
            # keypoints.data is a tensor of shape [N_people, 17, 3] (x, y, confidence)

            # Extract keypoints (landmarks) for the primary person (assumed index 0).
            # .cpu().numpy() converts the tensor to a numpy array.
            person_keypoints = yolo_results[0].keypoints.data[0].cpu().numpy()

            # --- POSE VISIBILITY CHECK (Adapted for YOLO) ---
            try:
                # Check key landmarks (Nose, left ankle, right ankle) confidence > 0.5
                ...
                # Enforce minimum confidence/visibility for processing
        ...
        if landmarks is not None and is_visible:
            ...
            try:
                ...
                # print(f"Error in frame processing: {e}") # Uncomment for debugging
...
def analyze_recorded_video(video_path, exercise_name):
    """Analyze a recorded video and provide comprehensive summary using YOLO."""
    global yolo_model
    if not yolo_model:
    ...
    while cap.isOpened():
        ...
        # Process with YOLO (REPLACING MediaPipe)
        yolo_results = yolo_model(image, verbose=False)
        landmarks = None
        ...
        if landmarks is not None:
            try:
                ...
                if len(processor_results) == 4:
                    rep_counter, exercise_state, feedback_text, _ = processor_results
                else:
                    rep_counter, exercise_state, feedback_text = processor_results
...
def display_live_ui(image, rep_counter, exercise_state, feedback_text, frame_width, frame_height, exercise_name):
    """Display UI elements for live mode, including centered title (Remains the same)."""
    overlay = image.copy()
    alpha = 0.6
...
def get_exercise_processor(exercise_name):
    """Return the appropriate exercise processor function (Remains the same)"""
    processors = {
...
def main():
    """Main application with mode selection (Remains the same)"""
    print("\n" + "=" * 60)
...
if __name__ == "__main__":
    main()
    # pose.close() # Removed MediaPipe cleanup